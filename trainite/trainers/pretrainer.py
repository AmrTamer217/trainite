...
from trainite.config import Config, dump_config
from trainite.datasets import build_dummy_dataloaders
from trainite.models import build_dummy_model
...
class PreTrainer:
    def __init__(
        ...
        self.model = model or build_dummy_model(config.model)
        ...
        if train_loader is None or val_loader is None:
            train_loader, val_loader = build_dummy_dataloaders(config)
    ...
    def _train_step(self, engine: Engine, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        if self.config.trainer.grad_clip_norm is not None:
            torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.config.trainer.grad_clip_norm)

        self.optimizer.step()
        return {"loss": loss.detach(), "logits": logits.detach(), "targets": targets.detach()}
    ...
    def _attach_handlers(self) -> None:
        self.engine.add_event_handler(Events.ITERATION_COMPLETED(every=self.config.trainer.log_every_steps), self._log_training)
        self.engine.add_event_handler(Events.EPOCH_COMPLETED, self._run_evaluations)
        self.engine.add_event_handler(Events.EPOCH_COMPLETED, self._save_last_checkpoint)
        self.val_evaluator.add_event_handler(Events.COMPLETED, self._update_best_checkpoint)
    ...
    def _run_evaluations(self, engine: Engine) -> None:
        ...
        self.writer.add_scalar("eval/train_token_accuracy", train_metrics["token_accuracy"], epoch)
        self.writer.add_scalar("eval/val_loss", val_metrics["loss"], epoch)
        self.writer.add_scalar("eval/val_token_accuracy", val_metrics["token_accuracy"], epoch)
